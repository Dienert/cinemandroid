...
//{{{ "private" functions
		...
		function calculateScrollPosition(){
			...
			$.each(wrap.find("a"), function(value) {
				if(value.hasClass(opts.playingClass)) {
					sumPrev = false;
				}	
				
				tempHeight = value.style.height;
				spliResult = tempHeight.split("px");
				
				heightInt = heightInt + parseInt(spliResult[0]);
				
				if(sumPrev){
					heightPrev = heightPrev + parseInt(spliResult[0]);
				}
			});
			
			var result = (heightPrev*330) / heightInt;
			
			
			alert(value.style.height);
		...
		function play(el, clip)  {
			...
			document.getElementById("playlist_grid").scrollTop = document.getElementById("playlist_grid").scrollTop + 100;
